...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ 
  ...
  return (
    <>
      <div className={`bg-spud-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group animate-scale-in border-2 border-spud-border hover:border-spud-orange ${!item.available ? 'opacity-60' : ''}`}>
        {/* Image Container with Badges */}
        <div className="relative h-48 bg-gradient-to-br from-spud-cream to-spud-light">
          {item.image ? (
            <img
              src={item.image}
              alt={item.name}
          ...
          <div className={`absolute inset-0 flex items-center justify-center ${item.image ? 'hidden' : ''}`}>
            <div className="text-6xl opacity-20 text-spud-orange">🥔</div>
          </div>
          
          {/* Badges */}
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-gradient-to-r from-spud-orange to-spud-hover text-spud-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg animate-pulse border border-spud-brown">
                SALE
              </div>
            )}
            {item.popular && (
              <div className="bg-gradient-to-r from-spud-brown to-spud-dark text-spud-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border border-spud-orange">
                ⭐ POPULAR
              </div>
            )}
          </div>
          ...
          {!item.available && (
            <div className="absolute top-3 right-3 bg-spud-dark text-spud-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg border border-spud-orange">
              UNAVAILABLE
            </div>
          )}
          ...
          {item.isOnDiscount && item.discountPrice && (
            <div className="absolute bottom-3 right-3 bg-spud-white/95 backdrop-blur-sm text-spud-orange text-xs font-bold px-2 py-1 rounded-full shadow-lg border border-spud-orange">
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-3">
            <h4 className="text-lg font-bold text-spud-brown leading-tight flex-1 pr-2">{item.name}</h4>
            {item.variations && item.variations.length > 0 && (
              <div className="text-xs text-spud-orange bg-spud-cream px-3 py-1.5 rounded-full whitespace-nowrap border border-spud-orange font-semibold">
                {item.variations.length} sizes
              </div>
            )}
          </div>
          
          <p className={`text-sm mb-4 leading-relaxed ${!item.available ? 'text-spud-border' : 'text-spud-dark'}`}>
          ...
          <div className="flex items-center justify-between mb-4">
              ...
              {item.isOnDiscount && item.discountPrice ? (
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <span className="text-2xl font-bold text-spud-orange">
                ...
                <div className="text-2xl font-bold text-spud-brown">
                  ₱{item.basePrice.toFixed(2)}
                </div>
              )}
              
              {item.variations && item.variations.length > 0 && (
                <div className="text-xs text-spud-border mt-1 font-medium">
                  Starting price
                </div>
              )}
            </div>
            ...
            <div className="flex-shrink-0">
              {!item.available ? (
                <button
                  disabled
                  className="bg-spud-border text-spud-border px-4 py-2.5 rounded-xl cursor-not-allowed font-medium text-sm border border-spud-border"
                >
                  Unavailable
                </button>
              ) : quantity === 0 ? (
                <button
                  ...
                  className="bg-gradient-to-r from-spud-orange to-spud-hover text-spud-white px-6 py-2.5 rounded-xl hover:from-spud-hover hover:to-spud-orange transition-all duration-200 transform hover:scale-105 font-bold text-sm shadow-lg hover:shadow-xl border border-spud-brown"
                >
          ...
          {item.addOns && item.addOns.length > 0 && (
            <div className="flex items-center space-x-1 text-xs text-spud-orange bg-spud-cream px-3 py-1.5 rounded-lg border border-spud-orange font-semibold">
              <span>+</span>
